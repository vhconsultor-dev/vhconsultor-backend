...
/// <summary>
/// Validador para el reporte de inventario de Vendor
/// </summary>
public class GenerateVendorInventoryReportValidator : AbstractValidator<GenerateVendorInventoryReportRequest>
{
    public GenerateVendorInventoryReportValidator()
    {
        RuleFor(x => x.ReportType)
            .NotEmpty()
            .WithMessage("El tipo de reporte es requerido")
            .Equal("GET_VENDOR_INVENTORY_REPORT")
            .WithMessage("El tipo de reporte debe ser GET_VENDOR_INVENTORY_REPORT");
        ...
        RuleFor(x => x.ReportOptions!.ReportPeriod)
            ...
            .Must(x => x == "DAY" || x == "WEEK" || x == "MONTH" || x == "QUARTER" || x == "YEAR")
            .WithMessage("El periodo del reporte debe ser DAY, WEEK, MONTH, QUARTER o YEAR")
            .When(x => x.ReportOptions != null);

        RuleFor(x => x.ReportOptions!.SellingProgram)
            .NotEmpty()
            .WithMessage("El programa de ventas es requerido")
            .When(x => x.ReportOptions != null);

        RuleFor(x => x.ReportOptions!.DistributorView)
            .NotEmpty()
            .WithMessage("La vista de distribuidor es requerida")
            .When(x => x.ReportOptions != null);
    }
